...
**Quantifying residence time—**In aquatic systems, residence time equals the ratio of volume to rate of flow, i.e. (V/Q = (V/V/time) = time). The inverse is known as dilution rate (1/time), i.e., the portion of particles or individuals lost or replaced per unit time. In experimental and engineered systems, dilution rate approximates growth rate and is used a master variable to control productivity and biomass (cite). For example, if dilution rate exceeds population growth rate, then populations can be washed out. But, if dilution rate is too slow (residence time is very long) then the supply of a vital resource (e.g. oxygen, nitrogen, carbon) may be too low to maintain actively growing populations.

**Residence time distributions—**


**Predictions of highly correlated residence times and residence time distributions—**
