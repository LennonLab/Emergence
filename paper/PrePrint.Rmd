---
title: "simplex: An individual-based ecological modeling platform"
...
A popular approach in ecology explicitly simulates the lives of individual organisms. Individual-based modeling (IBM) allows researchers to study ecological complexity as it emerges from individual-level interactions, to explicitly synthesize ecological theories and principles, and to track and record virtually any aspect of a system. I developed a novel IBM platform called simplex to explicitly simulate physiology, life history, resource-limited growth, fluctuating environmental dynamics, and community assembly and structure in both fluid and non-fluid environments. simplex accomplishes three primary tasks in keeping with its namesake, i.e., a generalization of the notion of a triangle to arbitrary dimensions. First, simplex assembles and runs stochastic ecological models from random combinations of over 20 state variables. Second, simplex stores the output of models and generates and stores animations. Third, simplex provides R Markdown files for analyzing simulated data. The simplex source code is freely available, includes the standard ODD description protocol, is open to community-based revision, includes unit testing and time testing, and has been developed to be run on a modest laptop setup using freely available open-source tools.
...
**Overview**--I developed simplex to accomplish three objectives. First, simplex assembles and runs individual-based ecological models from random combinations of state variables and processes. Second, simplex stores the output of models as R-style data frames and .csv files, and when the user chooses, generates and stores animations of models. Third, simplex provides R Markdown files for analyzing simulated data; these will eventually be offered in Python as well.
...
There are currently two R Markdown analysis files associated with simplex; these are located in the `tools/analyses/Rmd` directory. 
These files are primarily designed to help users understand how to import simplex files, to use R Markdown to analyze simplex data, and conduct descriptive and univariate statistics on simplex output. The first file, `BasicAnalysis.Rmd`, focuses on descriptive statistics, multiple univariate analyses, and variance partitioning to reveal relationships among simplex variables. The second file, `Diversity.Rmd`, focuses on the analysis of taxonomic diversity and abundance to include fitting species abundance models using the R-based `vegan` module. Each file generates a knitted pdf (includes code, text, and figures), see Supplement.

### Animations

I used the animation module in the matplotlib plotting library to generate avi formatted movies of simplex models. These can be viewed in any internet brower (https://github.com/LennonLab/simplex/tree/master/results/movies/examples). Currently, generating movies requires following three instructions in model/model.py:
...
Because generating animations requires computing time and memory, doing so can be computationally demanding. To quicken the process, comment out plt.show() on or near line 368.

# Discussion

The simplex platform assembles individual-based models from random combinations of state-variables and processes. This is done to explore a large degree of variation in ecological conditions and complexity. In this way, simplex allows ecologists or educators to examine whether the influence of a particular variable should be robust to a wide range of ecological conditions. In addition to its unique  modeling aspects, simplex includes analysis code that shows the user how to import and analyze the simulated data, and provides a basic demonstration of how to conduct variance partitioning and fitting of species abundance models.
