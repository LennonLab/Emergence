...
A popular approach in ecological modeling explicitly simulates the lives of individual organisms. Individual-based modeling (IBM) allows ecological complexity to be studied as it emerges from the individual-level, to explicitly synthesize theories and principles, and to track and record virtually any aspect of a system. I developed a novel IBM platform called simplex to explicitly simulate physiology, life history, resource-limited growth, fluctuating environmental dynamics, and community assembly and structure in both fluid and non-fluid environments. simplex accomplishes three primary tasks in keeping with its namesake (i.e., the generalized notion of a triangle to arbitrary dimensions). First, simplex assembles and runs stochastic ecological models from random combinations of over 20 state variables. Second, simplex stores the output of models and generates and stores animations. Third, simplex provides R Markdown files for analyzing simulated data. The simplex source code is freely available, includes the standard ODD description protocol, is open to community-based revision, includes unit testing and time testing, and has been developed to be run using modest hardware and freely available open-source tools.
...
**Individual organisms**--Individuals are distinguished by collections of elements within lists. Individuals undergo changes when randomly sampled from lists. Each specific position in the list corresponds to the same individual. For example, in simulating growth, a simplex model chooses an individual from the list of cell quotas (the probability of reproducing is determined by endogenous resources). The first position in this list as well in all other lists of individual attributes corresponds to the same individual.
	...
	The individual with ID of 1 has a cell quota of 0.1 and is 
  located at position x=45, y=765
