...
df2['Grow'] = df['active.avg.per.capita.growth'].groupby(df['sim']).mean()
df2['Maint'] = np.log10(df['active.avg.per.capita.maint'].groupby(df['sim']).mean())
df2['Disp'] = df['active.avg.per.capita.active.dispersal'].groupby(df['sim']).mean()
df2['RPF'] = df['dormant.avg.per.capita.rpf'].groupby(df['sim']).mean()
df2['Eff'] = df['active.avg.per.capita.efficiency'].groupby(df['sim']).mean()
df2['MF'] = df['dormant.avg.per.capita.mf'].groupby(df['sim']).median()
