...
```{r, results = 'hide', echo=FALSE, message = FALSE, warning = FALSE}
## Load Data
sim.data <- read.csv("~/GitHub/hydrobide/results/simulated_data/2015_June/26_June/SimData.csv")  
...
png(filename = '~/GitHub/hydrobide/results/figs/EandB_vs_ResTime.png',
      width = 700, height = 500, units = "px", pointsize = 17, bg = "black")

sp.lists <- c()


p1 <- ggplot(sim.data, aes(x=particle.tau, y=e.var, group=sim, colour=sim)) +
  scale_color_gradient(high="cyan") +
  geom_line() +
  theme_black() +
...
while(r < 61){
  print(r)
  myData <- list(read.csv("~/GitHub/hydrobide/results/simulated_data/2015_June/26_June/Species.csv", 
            header = FALSE, sep = ",", 
            
            skip=r, nrows=1))
  species.list <- c(species.list, myData)
  r <- r + 1
  }

sim.data$species.list <- species.list
num.sims <- max(sim.data$sim) + 1

i <- 0
while (i < num.sims){

  sp.sub <- subset(sim.data$species.list, sim == i)
  sp.beta <- c()
  j <- 2
  while(j <= length(sp.sub)){
    print(j)
    w.beta <- sp.turnover(sp.sub[j], sp.sub[j-1])
    if(w.beta != -1){
      sp.beta <- c(sp.beta, w.beta)
      }
    j <- j+1
    }
  i <- i+1
