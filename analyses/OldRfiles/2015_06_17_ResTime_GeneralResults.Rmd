...
plot(h.tau, log(N), xlab="Ecosystem Residence Time", 
     ylab="log(Total Abundance)", 
     main = "N vs. Tau", col='cornsilk4')

plot(h.tau, N, xlab="Ecosystem Residence Time", 
     ylab="Total Abundance", 
     main = "N vs. Tau", col='cornsilk4')

plot(h.tau, log(S), xlab="Ecosystem Residence Time", 
     ylab="log(Species Richness)", 
     main = "S vs. Tau", col='darkseagreen')

plot(h.tau, S, xlab="Ecosystem Residence Time", 
...
plot(x, y, xlab="Ecosystem Residence Time", 
     ylab="growth rate", 
     main = "Per capita growth vs. Tau",
     col='dodgerblue')

maint <- (S*sim.data$avg.per.capita.maint)/N
plot(h.tau, maint, xlab="Ecosystem Residence Time", 
     ylab="maintenance", 
...
sim.sub <- subset(sim.data, sim == 60)
height <- sim.sub$Height
width <- sim.sub$Width
flow.rate <- sim.sub$FlowRate

h.tau <- log((height * width)/flow.rate)
m.tau <- log(sim.sub$cell.tau)
t.tau <- log(sim.sub$particle.tau)
