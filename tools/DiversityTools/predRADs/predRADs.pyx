...
sys.path.append(mydir + "/tools/FeasibleFunctions")
...
sys.path.append(mydir + "/tools/macroeco_distributions")
import macroeco_distributions as md
sys.path.append(mydir + "/tools/predRADs")
import predRADs
sys.path.append(mydir + "/tools/mete")
import mete
sys.path.append(mydir + "/tools/pln")
import pln
...
def getPred(N, S, maxn, model, sample_size=1):
    ...
    for i in range(sample_size):
        ...
        if model == 'compositions':
            # Predicted from compositions (basically geometric distribution)
