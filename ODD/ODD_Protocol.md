...
The user runs a program that chooses random values for system state variables including whether disturbance, immigration, speciation, fluid dynamics, etc. will occur and at what rates. 

### Core simulation process
**simplex** models do not operate via definitive time steps (i.e. days, years, generations). Instead **simplex** models advance turnover of the environmental matrix according to the initial rate of flow. For example, if the initial rate of flow is 1.0, then inert particles would flow one unit distance through an environment that has no turbulence. After each iteration of flow each individual is given the chance to consume, grow, reproduce, starve, die, to disperse according to how the environment is flowing, and to actively move towards environmental optima.
...
What kinds of interactions among agents are assumed? Are there direct interac-tions in which individuals encounter and affect others, or are interactions indirect, e.g., via competition for a mediating resource? If the interactions involve communication, how are such communications represented? ###Stochasticity
What processes are modeled by assuming they are random or partly random? Is stochasticity used, for example, to reproduce variability in processes for which it is unim-portant to model the actual causes of the variability? Is it used to cause model events or be-haviors to occur with a specified frequency? ### Collectives
