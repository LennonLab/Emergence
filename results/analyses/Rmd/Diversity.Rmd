...
### 2.) How do species richness (S) and total abundance (N) relate to ecosystem residence time?

```{r, results = 'hide', echo=TRUE, message = FALSE, warning = FALSE, fig.width=7, fig.height=4}
tau <- (dat$width * dat$height)/dat$flow.rate

ggplot(dat, aes(x=tau, y=dat$total.abundance)) +
  geom_point(colour="red") +
  scale_x_log10() +
  scale_y_log10() +
  labs(x= "Residence time", y="Total abundance")

ggplot(dat, aes(x=tau, y=dat$species.richness)) +
  geom_point(colour="blue") +
  scale_x_log10() +
  scale_y_log10() +
  labs(x= "Residence time", y="Species richness")
```
